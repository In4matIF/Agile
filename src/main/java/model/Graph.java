...
public class Graph {
    ...
    public Graph(Plan p,Tour t) {
    	...
    	Dijkstra dijkstra = new Dijkstra(p); //Construction du graphe � partir du Plan
		
		for(Map.Entry<Integer,CrossingPoint> origin : t.getCrossingPoints().entrySet())
		{
			dijkstra.execute(origin.getValue().getIntersection()); //Application de dijkstra � partir de chaque CrossingPoint
			
			for(Map.Entry<Integer,CrossingPoint> destination : t.getCrossingPoints().entrySet())
			{
				if(!destination.equals(origin))
				{
					//R�cup�ration du chemin le plus court vers chaque CrossingPoint de destination
					...
					// Ajout de l'arr�te g�n�r�e au CrossingPoint de d�part
